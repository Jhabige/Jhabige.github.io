...
High thoughput technology is now routinely used to generate high dimenionsal (HD) data that allow for hundreds, thousands or even millions of null hypotheses to be tested. For example, in the "omics" the goal may be determine which among thousands of genes, bacteria, proteins, etc. are associated with a covariate or treatment group. This problem is challenging because 1) only a handful of samples are available for each test 2) it is necessary to adjust for multiplicity to safeguard against a high false positive rate 3) data may be discrete and difficult to model. I'm also recently interested in replicability analysis, where the goal is to identify phenomena that are replicated across multiple studies or populations, and some data may be subject to selection or publication bias.  

You can find full lists of my publications and some recent presentation outlining recent research with PhD students at OSU. The tutorial below will walk you though the two basic strategies for HD testing multiple null hypotheses - the highly cited [BH procedure](https://academic.oup.com/jrsssb/article/57/1/289/7035855) for false discovery rate control with p-values and an empirical bayes analogue that uses posterior null probability estimates, also sometimes called local false discovery rates.  Questions posed in the tutorial hint at specific research areas in the field.  Many remain unanswered!
